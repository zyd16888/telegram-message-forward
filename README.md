...
**聊天归档**是第三条独立旁路（会话历史拉取、检索与导出），不进入 Flow，也不写 `messages` 表。
...
- **聊天归档（旁路）**：把 Telegram 会话历史全量拉取到本地库，支持中文关键词检索（pg_trgm）与 JSONL / CSV / Markdown 导出，便于离线分析。归档不进 Flow、不产生投递，媒体默认只记录元信息
