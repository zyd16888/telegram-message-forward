...
const providerColumns: DataTableColumns<AIProvider> = [
  {
    ...
    key: 'name',
    render: (row) =>
      h('div', { class: 'provider-name' }, [
        h('span', row.name || row.id),
  ...
  { title: '默认模型', key: 'model' },
  {
    ...
    key: 'has_api_key',
    width: 100,
    render: (row) => h(NTag, { type: row.has_api_key ? 'success' : 'warning', bordered: false }, { default: () => (row.has_api_key ? '已配置' : '未配置') }),
  },
  {
    title: '操作',
    key: 'actions',
    ...
    render: (row) =>
      h('div', { class: 'row-actions' }, [
        ...
        h(NButton, { size: 'small', onClick: () => testProvider(row) }, { default: () => '测试' }),
        h(
          NButton,
          { size: 'small', type: 'error', secondary: true, disabled: providers.value.length <= 1, onClick: () => removeProvider(row) },
          { default: () => '删除' },
        ),
      ]),
  },
...
async function loadAll(): Promise<void> {
  loading.value = true
  try {
    const [ps, srcs, snks, meta, pvds, presetItems] = await Promise.all([
      aiApi.profiles.list(),
      sourcesApi.list(),
      sinksApi.list(),
      rulesApi.meta(),
...
function scheduleLabel(profile: AIDigestProfile): string {
  ...
  if (schedule.type === 'cron') return `Cron ${schedule.cron || ''} ${schedule.timezone || ''}`
  return '仅手动'
}
