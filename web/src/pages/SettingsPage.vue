...
import DataRetentionPanel from '@/components/settings/DataRetentionPanel.vue'
import MediaStoragePanel from '@/components/settings/MediaStoragePanel.vue'
...
async function loadTokens(): Promise<void> {
  ...
  try {
    ...
    message.error('加载 token 列表失败：' + errText(error))
  } finally {
    loading.value = false
  }
}
...
async function createToken(): Promise<void> {
  ...
  try {
    const result = await tokensApi.create(newTokenName.value.trim())
    createdToken.value = result.token
    newTokenName.value = ''
    message.success('已创建，请立即保存明文 token')
    await loadTokens()
  } catch (error) {
    message.error('创建失败：' + errText(error))
  }
}
...
async function revoke(id: number): Promise<void> {
  try {
    ...
    message.error('吊销失败：' + errText(error))
  }
}
...
const columns: DataTableColumns<ApiToken> = [
  ...
  { title: '名称', key: 'name' },
  { title: '状态', key: 'revoked', render: (row) => (row.revoked ? '已吊销' : '有效') },
  { title: '创建时间', key: 'created_at' },
  {
    ...
    render: (row) =>
      h(
        NButton,
        { size: 'small', type: 'error', disabled: row.revoked, onClick: () => revoke(row.id) },
        { default: () => '吊销' },
      ),
  },
]
...
<template>
  <n-space vertical size="large">
    ...
    <n-card title="API 鉴权状态">
      <n-alert :type="authStatus.type" :title="authStatus.title">{{ authStatus.desc }}</n-alert>
    </n-card>
    ...
    <n-card title="Token 管理（管理凭证维护）">
      <n-space vertical>
        <n-text depth="3">在此维护管理凭证：生成新凭证、吊销旧凭证。生成的明文 Token 仅显示一次。</n-text>
        <n-input-group>
