...
- 管理后台的配置类凭证以“方便用户查看和修改”为优先，API/UI 可回显已保存的渠道或外部系统配置凭证；不要为了通用脱敏规则把这类配置隐藏或替换为 `***`，除非用户另行明确要求。
- 管理 API 必须有最小鉴权。
- Telegram session、账号手机号、登录验证码、2FA 密码、数据库密码、加密主密钥、运行日志中的 token/secret 仍不得明文输出或提交。
